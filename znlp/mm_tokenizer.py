...
def tokenize(input_string=None):
    ...
    input_string = re.sub(u"([\u1000-\u104F])\s+(?=[\u1000-\u104F])", u"\1", input_string)

    ## add a space between digits and non digits and non dot and non comma
    input_string = re.sub(u"([^\u1040-\u1049\u002E\u002C])([\u1040-\u1049])", u"\1`\2", input_string)
    input_string = re.sub(u"([\u1040-\u1049])([^\u1040-\u1049\u002E\u002C])", u"\1`\2", input_string)

    ## tokenize ။ and ၊
    input_string = re.sub(u"(\u104A|\u104B)", u"`\1`", input_string)

    ## auk ka mit and a tat reordering
    input_string = re.sub(u"\u103A\u1037", u"\u1037\u103A", input_string)

    input_string = re.sub(u"([\u1000-\u102A\u103F\u104C-\u104F])", u'`\1', input_string)
    input_string = re.sub(u"`([\u1000-\u1021])([\u1039\u103A])", u'\1\2', input_string)

    input_string = re.sub(u"`([\u1000-\u1021])([\u1037\u103A])", u'\1\2', input_string)
    input_string = re.sub(u"([\u1039])`([\u1000-\u1021])", u'\1\2', input_string)
    input_string = re.sub(u"([\u1000-\u103F\u104C-\u104F])([!-\u00D7\u1040-\u104B\u2018-\u201D])", u'\1`\2', input_string)
