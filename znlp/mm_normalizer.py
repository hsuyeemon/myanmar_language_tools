...
def normalize(input_string=None):
    ...
    input_string = re.sub(u"([\u102B-\u1035]+)([\u103B-\u103E]+)", r"\2\1", input_string);

    # ## reordering myanmar storage order
    input_string = re.sub(u"([\u102D\u102E\u1032]{0,})([\u103B-\u103E]{0,})([\u102F\u1030]{0,})([\u1036\u1037\u1038]{0,})([\u102D\u102E\u1032]{0,})", r"\2\1\5\3\4", input_string)
    input_string = re.sub(u"(^|[^\u1000-\u1021\u103B-\u103E])(\u1031)([\u1000-\u1021])((?:\u1039[\u1000-\u1021])?)([\u103B-\u103E]{0,})", r"\1\3\4\5\2", input_string)

    ## for aukmyit and SIGN AA
    input_string = re.sub(u"\u1037\u102C", u"\u102C\u1037", input_string)

	## For Latest Myanmar 3
    input_string = re.sub(u"\u103A\u1037", u"\u1037\u103A", input_string)
    input_string = re.sub(u"\u1036\u102F", u"\u102F\u1036", input_string)
    ...
    input_string = re.sub(u"([\u102B-\u103E])\1+", r"\1", input_string)

    ## these duplicates based on document frequency errors
    ## remove double or more SIGN MEDIAL WA and HA
    input_string = re.sub(u"(\u103D\u103E)+", u"\u103D\u103E", input_string)

    ## remove double or more VOWEL SIGN U and ANUSVARA
    input_string = re.sub(u"(\u102F\u1036)+", u"\u102F\u1036", input_string)

    ## remove double or more SIGN 1 and SIGN U
    input_string = re.sub(u"(\u102D\u102F)+", u"\u102D\u102F", input_string)

    ## fixed wrong spelling
    input_string = re.sub(u"([\u102D\u102E])\u1030", u"\1\u102F", input_string)

    ## For the case of ဖံွ့ဖြိုး
    input_string = re.sub(u"([\u1000-\u1021])(\u1036)(\u103D)(\u1037)", r"\1\3\2\4", input_string)

    ## For the case of အိနိ္ဒယ
    input_string = re.sub(u"([\u1000-\u1021])(\u102D)(\u1039)([\u1000-\u1021])", r"\1\3\4\2", input_string)
    input_string = re.sub(u"([\u1000-\u1021])(\u1036)(\u103E)", r"\1\3\2", input_string)
    ...
    input_string = re.sub(u"\u1037\u102F", u"\u102F\u1037", input_string)

    ## reorder Sign Wa and  ANUSVARA
    input_string = re.sub(u"\u1036\u103D", u"\u103D\u1036", input_string)

    ## reorder for သင်္ဘော
    input_string = re.sub(u"(\u1004)(\u1031)(\u103A)(\u1039)([\u1000-\u1021])", r"\1\3\4\5\2", input_string)

    ## type error
    input_string = re.sub(u"(\u102D)(\u103A)+", r"\1", input_string)

    ## fix nya lay that and Sign U
    input_string = re.sub(u"\u1025\u103A", u"\u1009\u103A", input_string)

    ## Type Error (reorder)
    input_string = re.sub(u"([\u1000-\u1021])(\u1031)(\u103D)", r"\1\3\2", input_string)

    ## Type Error (reorder)
    input_string = re.sub(u"([\u1000-\u1021])(\u1031)(\u103E)(\u103B)", r"\1\3\4\2", input_string)
